...
export namespace MIDIOut {
  ...
  export const MMSSToms = (mmss: string): number => {
    ...
    throw new Error(
      `${mmss} does not match the required syntax: /^[0-9]+:[0-5][0-9]$/. Valid examples: 1:23, 10:59, 0:12`
    )
  }

  export const validateMIDIStartTimeInput = (
    durationMS: number,
    durationMMSS: string,
    mmss: string
  ): string | undefined => {
    try {
      ...
      if (ms > durationMS) {
        throw new Error(
          `Duration ${mmss} is longer than duration of actual MIDI file ${durationMMSS}`
        )
      }
    } catch (err) {
      return String(err)
  ...
  const getMidiFilePathFromWindow = async () => {
    ...
    if (!activeTextEditor) {
      throw new Error(
        `No active \`lilypond\`text editor open (Please click inside a \`lilypond\` text document to make it active)`
      )
    }
    ...
    const getFirstMidiTypeFilePath = async (baseFilePath: string) => {
      ...
      const filePaths = fs
        ...
        .filter(
          (p: string) => p.substr(0, baseFilePath.length) === baseFilePath
        )
    ...
    const baseFilePath = stripFileExtension(
  ...
  const pollMIDIStatus = async () => {
    ...
    if (
      positionMS === 0 &&
      ((MIDIOutState.player && MIDIOutState.playing) || MIDIOutState.paused)
    ) {
      stopMIDI()
    } else {
      vscode.window.setStatusBarMessage(
        `Playing \`${MIDIOutState.currMidiFilePath}\`: ${getPlayTimeStamp(
          durationMS,
          positionMS
        )}`
      )
  ...
  export const stopMIDI = async () => {
    try {
      if (
        (MIDIOutState.player && MIDIOutState.playing) ||
        MIDIOutState.paused
      ) {
  ...
  export const pauseMIDI = async () => {
    try {
      if (MIDIOutState.player && MIDIOutState.playing && !MIDIOutState.paused) {
        ...
        vscode.window.setStatusBarMessage(
          `Paused MIDI: \`${
            MIDIOutState.currMidiFilePath
          }\`: ${getPlayTimeStamp(durationMS, positionMS)}`
        )
        if (timeout) {
          clearTimeout(timeout)
        }
  ...
  export const resetMIDI = async (mute = false) => {
    if ((MIDIOutState.player && MIDIOutState.playing) || MIDIOutState.paused) {
      stopMIDI()
    }
    if (timeout) {
  ...
  export const initMIDIStatusBarItems = () => {
    {
      const playBtn = vscode.window.createStatusBarItem(
    ...
    {
      const playFromBtn = vscode.window.createStatusBarItem(
        vscode.StatusBarAlignment.Left,
        99
      )
    ...
    {
      const pauseBtn = vscode.window.createStatusBarItem(
        vscode.StatusBarAlignment.Left,
        98
      )
